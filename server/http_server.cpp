#include "http_server.hpp"

mg_serve_http_opts HttpServer::m_server_option;
unordered_set<mg_connection*> HttpServer::m_websocket_session_set;


static bool route_check(http_message *http_msg, char *route_prefix){
    if(mg_vcmp(&http_msg->uri, route_prefix)==0){
        return true;
    }
    return false;
}

static int isWebsocket(mg_connection* conn){
    return conn->flags & MG_F_IS_WEBSOCKET;
}

void HttpServer::init(string port, string root){
    m_port = port;
    m_server_option.enable_directory_listing = "yes";
    m_server_option.document_root = root.c_str();
}


bool HttpServer::start(){
    mg_mgr_init(&m_mgr, nullptr);
    mg_connection *connection = mg_bind(&m_mgr,m_port.c_str(), HttpServer::onHttpWebsocketEvent);
    if(connection==nullptr){
        return false;
    }
    mg_set_protocol_http_websocket(connection);
    cout<< "启动webserver , 监听端口"<<m_port<<endl;
    while (true)
    {
        mg_mgr_poll(&m_mgr, 500); // 循环处理所有的socket
    }
    return true;
}


void HttpServer::onHttpWebsocketEvent(mg_connection *connection, int event_type, void* event_data)
{
    if(event_type == MG_EV_HTTP_REQUEST){
        http_message *http_req = (http_message *) event_data;
        handleHttpEvent(connection, http_req);

    }else if(event_type== MG_EV_WEBSOCKET_HANDSHAKE_DONE||
                event_type==MG_EV_WEBSOCKET_FRAME||
                event_type==MG_EV_CLOSE)
    {
        websocket_message *ws_msg = (struct websocket_message *) event_data;
        handleWebsocketEvent(connection, event_type, ws_msg);
    }
}


void HttpServer::handleHttpEvent(mg_connection* conn, http_message* http_req){
    std::string req_str = string(http_req->message.p, http_req->message.len);
    cout<< "got request: "<< req_str<<endl;

    string url = std::string(http_req->uri.p, http_req->uri.len);
    string body = string(http_req->body.p, http_req->body.len);
    if(route_check(http_req,"/")){
        mg_serve_http(conn, http_req, m_server_option);
        
    }else if(route_check(http_req, "/api/hello"))
    {
        sendHttpRsp(conn, "hello world!!");
    }else if(route_check(http_req,"/api/sum"))
    {
            char n1[100], n2[100];
            double result;
            mg_get_http_var(&http_req->body, "n1", n1, sizeof(n1));
            mg_get_http_var(&http_req->body, "n2", n2, sizeof(n2));
            result = strtod(n1, NULL) + strtod(n2, NULL);
            sendHttpRsp(conn, std::to_string(result));
    }
    else{
        mg_printf(conn,"%s",
                    "HTTP/1.1 501 Not Implemented\r\n" 
                    "Content-Length: 0\r\n\r\n");
    }
}

void HttpServer::sendHttpRsp(mg_connection *connection, std::string rsp)
{
	mg_printf(connection, "%s", "HTTP/1.1 200 OK\r\nTransfer-Encoding: chunked\r\n\r\n");
	// 以json形式返回
	mg_printf_http_chunk(connection, "{ \"result\": %s }", rsp.c_str());
	// 发送空白字符快，结束当前响应
	mg_send_http_chunk(connection, "", 0);
}


void HttpServer::handleWebsocketEvent(mg_connection* conn, int event_type, websocket_message* ws_msg){
    if(event_type==MG_EV_WEBSOCKET_HANDSHAKE_DONE)
    {
        printf("client websocket connected\n");
		// 获取连接客户端的IP和端口
        char addr[32];
        mg_sock_addr_to_str(&conn->sa, addr, sizeof(addr), MG_SOCK_STRINGIFY_IP|MG_SOCK_STRINGIFY_PORT);
        printf("client addr: %s\n", addr);
        
        m_websocket_session_set.insert(conn);
        sendWebsocketMsg(conn, "client websocket connected");
    }
    else if(event_type==MG_EV_WEBSOCKET_FRAME)
    {   
        mg_str received_msg = {(char*) ws_msg->data, ws_msg->size};
        char buff[1024] = {0};
        strncpy(buff, received_msg.p, received_msg.len);
        cout << "received msg: "<<string(buff)<<endl;
        sendWebsocketMsg(conn, "send your msg back: " + std::string(buff));
    }
    else if(event_type==MG_EV_CLOSE){
        if(isWebsocket(conn)){
            // 移除连接
            cout<<"client websocket closed"<<endl;
			if (m_websocket_session_set.find(conn) != m_websocket_session_set.end())
				m_websocket_session_set.erase(conn);
        }
    }
}

void HttpServer::sendWebsocketMsg(mg_connection* conn, string msg){
    mg_send_websocket_frame(conn, WEBSOCKET_OP_TEXT, msg.c_str(), strlen(msg.c_str()));
}

bool HttpServer::close(){
    mg_mgr_free(&m_mgr);
    return true;
}